...
$(document).ready ->
  ...
  $("#update_contact_field .submit").click ->
    new_contact_name = $("#update_contact_field .create_contact_text").val()
    if new_contact_name == ""
      alert("請輸入聯繫方式名稱")
      return
    $.ajax
      ...
      success: (data, textStatus, jqXHR) ->
        if data.response.success == true
          $("#update_contact_field table").append("<tr><td>" + new_contact_name + "</td><td></td></tr>")
          $("#update_contact_field .create_row").appendTo("#update_contact_field table")
          $("#update_contact_field .create_contact_text").val("")
